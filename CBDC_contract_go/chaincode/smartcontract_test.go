...
func TestCreateAsset(t *testing.T) {
	...
	assetTransfer := chaincode.SmartContract{}
	err := assetTransfer.CreateAsset(transactionContext, "asset1", 1622548800, 1000, "BuyerA", "BuyerASignature", "MerchantA")
	require.NoError(t, err)

	chaincodeStub.GetStateReturnsOnCall(0, nil, nil)      // 模拟资产不存在
	chaincodeStub.GetStateReturnsOnCall(1, []byte{}, nil) // 模拟资产存在
	err = assetTransfer.CreateAsset(transactionContext, "asset1", 1622548800, 1000, "BuyerA", "BuyerASignature", "MerchantA")
...
func TestUpdateAssetDeliveryTimeStamp(t *testing.T) {
	// 初始化模拟对象和变量
	chaincodeStub := &mocks.ChaincodeStub{}
	transactionContext := &mocks.TransactionContext{}
	transactionContext.GetStubReturns(chaincodeStub)
	...
	chaincodeStub.GetStateReturns(bytes, nil) // 模拟资产存在
	assetTransfer := chaincode.SmartContract{}
	err = assetTransfer.UpdateAssetDeliveryTimeStamp(transactionContext, "asset1", 1622549900)
	require.NoError(t, err)

	chaincodeStub.GetStateReturns(nil, nil) // 模拟资产不存在
	err = assetTransfer.UpdateAssetDeliveryTimeStamp(transactionContext, "asset1", 1622549999)
	require.EqualError(t, err, "the asset asset1 does not exist")
}
